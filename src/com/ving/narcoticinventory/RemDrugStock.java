...
public class RemDrugStock extends Activity implements OnItemClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mContext = this;
		setContentView(R.layout.activity_remdrugstock);
		LinearLayout background = (LinearLayout) findViewById(R.id.remStockBackground);
        ...
        for (Drug drug : myApp.getDrugList()) {
        	countItems.add(drug.name() + " -- No Quantity Removed");
        }
	...
	public void remDrugOnClick (View target) {
		...
		case R.id.save:
			if ((nurse1Value.equals("")) || (nurse2Value.equals(""))) {
				...
				AlertDialog.Builder dialog = new AlertDialog.Builder(mContext);
	            dialog.setTitle("Two Different Nurses Must Verify Removals");
	            dialog.setCancelable(false);
	            dialog.setPositiveButton("Ok",new DialogInterface.OnClickListener() {
	                public void onClick(DialogInterface dialog,int id) {
	                	dialog.cancel();
				...
				sigCollected = 0;
				Intent sigIntent = new Intent(RemDrugStock.this, CaptureSignature.class);
	...
	public void onItemClick(AdapterView<?> arg0, final View target, final int position,
    	...
    	AlertDialog.Builder dialog = new AlertDialog.Builder(mContext);
    	View dialogView = getLayoutInflater().inflate(R.layout.dialog_remstock,null);
        ...
        final RadioButton remExpired = (RadioButton)dialogView.findViewById(R.id.remExpired);
        final RadioButton remRecall = (RadioButton)dialogView.findViewById(R.id.remRecall);
        final RadioButton remOther = (RadioButton)dialogView.findViewById(R.id.remOther);
        remExpired.setChecked(false);
        remRecall.setChecked(false);
        remOther.setChecked(false);
        final EditText remOtherText = (EditText)dialogView.findViewById(R.id.remOtherText);
        ...
        pos.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				if (qty != Transaction.UNDEF) {
					if ((! remExpired.isChecked()) && (! remRecall.isChecked()) && (! remOther.isChecked())) {
						AlertDialog.Builder dialog = new AlertDialog.Builder(mContext);
						dialog.setTitle("No Removal Reason Selected");
						dialog.setMessage("Please select the reson for removal.");
						dialog.setCancelable(false);
						...
						dialog.show();
					} else if ((remOther.isChecked()) && (remOtherText.getText().toString().equals(""))) {
						AlertDialog.Builder dialog = new AlertDialog.Builder(mContext);
						dialog.setTitle("Other Reason not Provided");
						dialog.setMessage("Please enter the text for the reason.");
						dialog.setCancelable(false);
						dialog.setPositiveButton("OK",
								new DialogInterface.OnClickListener() {
									public void onClick(DialogInterface dialog, int id) {
						...
						if (remExpired.isChecked()) {
							removalReason = getResources().getString(R.string.expired);
						} else if (remRecall.isChecked()) {
							removalReason = getResources().getString(R.string.recall);
						} else {
							removalReason = remOtherText.getText().toString();
						}
						countItems.setValues(position, drug.name() + " -- Removed " + qty + ": " + removalReason, qty);
						countItems.setOverride(position, removalReason);
        ...
        case CaptureSignature.SIGNATURE_ACTIVITY: 
            if (resultCode == RESULT_OK) {
                ...
                if (sigCollected == 1) {
                	...
                	passTitle.setText("Save Stock Removal Transactions");
                	TextView passMessage = (TextView) dialogView.findViewById(R.id.getPassMessage);
                	passMessage.setText("To save the stock removal transactions, please enter the Administrator Password below and click on the Continue button.");
            		        ...
            		        if (myApp.getAdmin().verify(password.getText().toString())) {
			                	...
			                	for (int i=0;i<countItems.size();i++) {
			                		if ((countItems.valueSet(i)) && (countItems.quantity(i) > 0)) {
			                			myApp.getDrug(i).transaction(countItems.comment(i), countItems.quantity(i), nurse1Value, nurse2Value, nurse1SigFile, nurse2SigFile);
